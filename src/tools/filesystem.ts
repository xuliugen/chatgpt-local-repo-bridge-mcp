...
export function registerFilesystemTools(server: McpServer): void {
  ...
  server.registerTool(
    'list_directory',
    {
      ...
      description: '列出指定目录的内容，包括文件和子目录。可选择递归列出。',
      inputSchema: {
  ...
  server.registerTool(
    ...
    {
      title: 'Read File',
      description: '读取指定文件的内容。支持按行号范围读取部分内容。' +
        '默认只读取前 500 行以防止 Token 浪费，如需完整内容请设置 endLine 为大数字。' +
        '返回内容带有行号前缀，方便后续用 edit_file 定位。',
      inputSchema: {
        path: z.string().describe('要读取的文件路径'),
        startLine: z.number().optional().describe('起始行号 (1-based)，不指定则从头开始'),
        endLine: z.number().optional().describe('结束行号 (1-based)，默认为 500'),
      },
    },
    async ({ path: filePath, startLine, endLine }): Promise<CallToolResult> => {
      ...
      const end = Math.min(endLine ?? 500, totalLines);
  ...
  server.registerTool(
    ...
    {
      title: 'Write File',
      description: '创建新文件或完全覆盖已有文件的内容。会自动创建所需的父目录。',
      annotations: {
        title: 'Write File',
    ...
    async ({ path: filePath, content }): Promise<CallToolResult> => {
      ...
      logger.warn(`write_file: ${resolved} (${content.length} bytes)`);
      ...
      return {
        content: [{ type: 'text', text: `文件已成功写入: ${resolved} (${content.length} 字节)` }],
      };
    }
  );
  ...
  server.registerTool(
    ...
    {
      title: 'Edit File',
      description: '局部编辑文件，支持两种定位模式:\n' +
        '1. 文本匹配模式: 通过 oldText/newText 搜索替换 (oldText 必须唯一匹配)\n' +
        '2. 行号模式: 通过 startLine/endLine 指定要替换的行号范围\n' +
        '两种模式可以在同一次操作中混合使用。',
      annotations: {
        title: 'Edit File',
      ...
      inputSchema: {
        ...
        edits: z.array(
          z.object({
            oldText: z.string().describe('要被替换的原始文本 (文本匹配模式，与 startLine 二选一)'),
            newText: z.string().describe('替换后的新文本'),
            startLine: z.number().optional().describe('替换起始行号 (行号模式，1-based)'),
            endLine: z.number().optional().describe('替换结束行号 (行号模式，1-based，包含该行)'),
          })
        ).describe('替换操作列表。每项可以用 oldText 做文本匹配，或用 startLine+endLine 做行号定位'),
      },
    },
      ...
      for (const edit of edits) {
        ...
        if (startLine !== undefined && endLine !== undefined) {
          ...
          if (start < 0 || end > lines.length || start >= end) {
            return {
              content: [{ type: 'text', text: `编辑失败: 行号范围无效 (startLine=${startLine}, endLine=${endLine}, 文件共 ${lines.length} 行)` }],
              isError: true,
            };
          }

          const replacedText = lines.slice(start, end).join('\n');
          lines.splice(start, end - start, ...newText.split('\n'));
          appliedEdits.push(`行 ${startLine}-${endLine}: 替换了 ${end - start} 行`);
        } else if (oldText !== undefined) {
          // --- 文本匹配模式 ---
          const joined = lines.join('\n');
          const count = joined.split(oldText).length - 1;
          ...
          return {
            content: [{ type: 'text', text: `编辑失败: 每项编辑必须提供 oldText (文本匹配) 或 startLine+endLine (行号模式)` }],
            isError: true,
          };
        }
      }
